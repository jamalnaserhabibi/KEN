    ...
    <script type="text/javascript">
        function delete_id(id) //delete confirmatation
        {
            if (confirm('Do you want to delete (' + id + ') id?')) {
                window.location.href = 'deletebook.php?bid=' + id;
            }
        }
    </script>
</head>
            ...
            <div class="data" id="ajxdata">
                ...
                if (isset($result)) {
                    ...
                    while ($row = $result->fetch()) {
                        ...
                        echo '<td>' . $row[6] . '</th>';
                        echo '<td><center><a class="lbtn update" href="updatebook.php?bid=' . $row[0] . '&&bname=' . $row[1] . '&&bimage=' . $row[2] . '&&bcurri=' . $row[4] . '&&bgradeid=' . $row[3] . '&&bremark=' . $row[6] . '&&bprice=' . $row[5] . '">Update</a>';
