...
if (isset($_GET['subid'])) {
    ...
    $subname = $_GET['sname'];
    $subdep = $_GET['subdep'];
    $subgradeid = $_GET['subgradeid'];
    $subremarks = $_GET['subremarks'];
} else {
    echo 'Cant get data';
}
?>
...
<head>
    ...
    <title>Update <?php echo $subname ?></title>
</head>
<body>
    ...
    <center>
        ...
        <div class="inputs">
            ...
            <form method="post" action="" enctype="multipart/form-data">
                <input value="<?php echo $subname ?>" class="ttt" type="text" name="subname" placeholder="Subject Name"
                       required=""><br>
                <!-- filling select from department table -->
                <select name="subdepart" required="">
                    <!-- <option values="" disabled selected hidden>Select Related Department</option>  -->
                    <?php
                    require_once("connection.php");
                    try {
                    ...
                    if (isset($result)) {
                        while ($row = $result->fetch()) {
                            if ($row[1] == 1) {
                                ...
                                if ($row[1] == $subgradeid) echo '<option value="' . $row[0] . '" selected>1st Grade</option>';;
                            } elseif ($row[1] == 2) {
                                echo '<option value="' . $row[0] . '">2nd Grade</option>';
                                if ($row[1] == $subgradeid) echo '<option value="' . $row[0] . '" selected>2nd Grade</option>';;
                            } elseif ($row[1] == 3) {
                                echo '<option value="' . $row[0] . '">3rd Grade</option>';
                                if ($row[1] == $subgradeid) echo '<option value="' . $row[0] . '" selected>3rd Grade</option>';;
                            } elseif ($row[1] > 3) {
                                echo '<option value="' . $row[0] . '">' . $row[1] . 'th Grade</option>';
                                if ($row[1] == $subgradeid) echo '<option value="' . $row[0] . '" selected>' . $row[1] . 'th Grade</option>';;
                            } else {
                                echo '<option value="' . $row[0] . '">' . $row[1] . ' </option>';
                                if ($row[1] == $subgradeid) echo '<option value="' . $row[0] . '" selected>' . $row[1] . '</option>';;
                            }
                        }
                    }
                    ?>
                ...
                <!-- fill end -->
                <input value="<?php echo $subremarks ?>" class="ttt" type="ttt" name="subremarks"
                    ...
                    try {
                        if (isset($_POST['subdepart']) && isset($_POST['subgradeid'])) {
                            if (isset($_POST['btn-submit'])) {
                                try {
                                    $query = "UPDATE `tblsubject` SET `subname`=:nam,`subdepid`=:dep,`subgradeid`=:subgradeid,`subremarks`=:rem WHERE `subid`=$subid";
